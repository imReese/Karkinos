...
For product and UI work, consult `DESIGN.md` when relevant.
...
Use `docs/ENGINEERING.md` for the current test strategy and local validation commands. `.github/workflows/ci.yml` is the source-verification authority; `Promotion Gate` is its stable promotion result.
...
The trusted promotion workflow may fast-forward only the **current** `dev` head. That exact SHA must have a successful `Promotion Gate` from its own `dev` push CI. A red or pending tip waits; never promote an older green ancestor and never run stale `main` CI definitions against newer `dev` source as authorization.

The privileged promotion controller must execute trusted `main` controller code only. It may inspect the exact commit's gate metadata and perform the non-force ref update; it must not checkout or execute candidate code with write credentials.

Immediately before moving `main`, revalidate the exact `Promotion Gate`, current `dev`, current `main`, and fast-forward ancestry. Any disagreement fails closed.
