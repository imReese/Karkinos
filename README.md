...
事件可哈希链回放；接受上下文仍不会启动模型或 workflow。没有真实模型调用、API Key、
厂商绑定、后台 AI 任务或实盘入口。
...
- AI-native research runtime foundation with provider/model/agent-role
  ...
  chain from an explicitly opened Strategy Lab panel. Context acceptance still
  starts no model or workflow. The runtime uses only a deterministic local
  fixture provider and exposes no model API, OMS action, or broker authority.
