...
- Return calendar cockpit view: inspect one month by day, one year by month, or every year from audited attribution data. Calendar cells use market PnL, keep cash deposits/withdrawals as external-flow context, and mark periods with incomplete historical price coverage instead of presenting flat zero returns.
