...
- v0.2.5 验证证据：2,500 项 Python 测试与 697 项 Web 测试通过；Web 格式检查、生产构建、文档健康、
...
- owner 授权的收盘后 AI shadow 研究绑定最新持久化日线与完整 valuation/ledger identity：每个市场日期一次、原子 capped provider-call claim、记录 Token 用量但 Karkinos 不设每日累计 Token 上限，DeepSeek 仅基于严格脱敏账户风险/配置 allowlist 提出 Formula DSL 假设、本地校验与 canonical 成本后 rolling OOS、证据 critique、Web 对比/通知及精确人工-only canonical `paper_shadow` 晋级；确定性晋级门把实际有序 timestamp/OHLCV、对齐 OOS、有界 AST 参数网格、冻结行情状态、回撤/换手、日线容量/流动性、经券商账单对账的账户专属税费及 critique 绑定为可重算证据，内置费用估算明确不能晋级；Account Truth 晋级与就绪以持久化现金/持仓快照时间推导实际采集时间，`imported_at` 仅保留审计，缺失、跨日、晚于快照的财务事件、未来时间或过期快照会统一 fail closed；保留策略在晋级/票据时重查所有源，精确 plan/paper/actual 对比不通过则下一批 no-action；policy/Kill Switch/证据/OOS/成本/单次模型技术限制/调用次数异常均 fail closed，绝对账户金额和 valuation/ledger 标识在外发前删除，且不存在生产策略、OMS、ledger、broker 或资本权限；
...
- Overview 复核队列、Operations 塔台与专用 `/operations` 证据中心消费同一 canonical daily-operations projection；每个非正常
  子系统现有 deterministic、可感知 drift 的 attention fingerprint、安全下一步与证据解除条件，确认
  本身不能清除；证据中心先核验所有无权限标记再允许安全下钻，AI capture 只接收同一只读 payload。风险影响为 high，因为 canonical builder 同时
