...
[中文](broker-adapter-release-review.zh.md) | [Lifecycle ingestion](broker-order-lifecycle-ingestion.en.md) | [Roadmap](ROADMAP.md)
...
`karkinos.broker_adapter_release_manifest.v1` before collector preparation and
again before a prepared run is committed after restart.
...
Canonical tables are `broker_adapter_release_manifests` and
`broker_adapter_release_review_events`. Repeating the same review id and exact
...
2. prepare resolves the latest persisted review by `release_evidence_ref`;
...
4. a prepared run repeats the same verification before lifecycle persistence
   and cursor commit;
5. missing, rejected, revoked, tampered, or drifted review evidence blocks the
   run without creating a lifecycle fact or advancing the cursor.
...
- **Validation:** deterministic fixtures cover preview, sensitive/unknown
  ...
  review, and revoke-between-prepare-and-commit.
