...
Unknown outcome 只允许查询。单独打开的 ledger-posting 复核现可在无需手改数据库的情况下完成既有
签名式 posting boundary：preview 保持只读，私钥留在离线本地签名器，最终 apply 在原子写事务内
重新核验全部证据。投影本身不联系 provider、不修改账本，也不授予 submit、cancel、resume 或
capital authority。签名式 submission、terminal-clearance 与 correction UI、cancel/recovery action，
以及完整的无需手改数据库旅程仍未完成。
