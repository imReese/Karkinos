...
| v1.8 adapter acceptance | Provider-neutral 基础已实现 | 版本化 manifest、deterministic local conformance 证据、capability/boundary matrix、append-only accept/reject/revoke review 与 live collector 精确绑定；尚未选择或注册真实 provider |
...
与 revocation 复核。该 suite 验证 Karkinos 契约，不验证真实 adapter。真实 provider 选择、实际
