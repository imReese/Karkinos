...
partial-fill/cancel 竞态，且不注册 adapter 或联系 provider。Deterministic evidence 也可进入单独
签名的精确终态 clearance；已有精确 open lifecycle 还能生成 copy-only 人工撤单资料。真实 adapter
conformance、单独签名的显式撤单命令与真实环境 recovery 证据仍属于发布工作。
