...
Stage 3.15 已增加 QMT 专用的“单笔订单生命周期本地证据适配基础”，但没有增加 QMT 连接。
`scripts/import_qmt_order_lifecycle.py` 默认只做 preview；只有显式 `--record` 加
`record_qmt_order_lifecycle_evidence_without_execution_authority` acknowledgement 才会把
...
证据仍不能替代独立 broker statement、最新 Account Truth 和 Stage 3.14 人工签名。生产仍无
collector、write adapter、release provider、可执行撤单或 pilot 权限；正式 pilot 前必须实现并
独立审查真实 QMT callback/poll collector，完成断连、重启、重复、乱序和恢复 soak。
