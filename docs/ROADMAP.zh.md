...
| AI 原生 Phase 1-1.18 + 结果/复盘桥接 | 已实现 | provider-neutral、证据绑定的研究、记忆、Formula DSL/回测/批判、人工显式选择的 canonical 策略贡献捕获，以及 signal→执行事实→人工决策后复盘；不具备交易权限 |

Account Truth 和对账继续作为强制安全门，但不再是下一阶段产品主线。Phase 1.18 已按一次明确
授权完成公式研究纵向闭环；v1.8 仍是当前产品目标。新增的只读结果捕获和人工复盘桥接不启动
Phase 1.19，也不会自动进入 AI prompt 或长期记忆。进一步 AI、语义记忆扩展和自动 prompt
...
**已实现基础：** deterministic evidence 已可进入单独签名的精确终态 clearance，覆盖完整成交、
零成交撤单和部分成交后撤单；它只记录实际 fills，绝不修改生产账本。仍开放的 partial fill 继续
阻断。对于已有精确持久化 open/partial lifecycle 的订单，操作员现在可预览并导出 provider-neutral
的人工撤单证据包；它绑定 broker/client 双重订单 ID 与最新 observation，不联系券商也不修改任何
状态，仍必须由更新的 lifecycle 证据证明撤单结果。真实 provider conformance、单独签名的显式
撤单命令和真实环境 recovery 证据仍属于发布工作。
...
replay 推导的 delta，验证独立离线 proof，再 exactly once 地追加补偿事件。更广的
fault-injection acceptance 与真实 provider 证据仍未完成，因此 M3 尚不是发布完成声明。
...
4. **基础部分已实现：** query/callback 生命周期、签名的完整成交/partial-cancel/零成交撤单
   精确终态对账，以及具备重复点击/restart 防护的签名式 query-only unknown recovery；真实 cancel
   与真实 adapter 的 recovery 证据仍待完成。
