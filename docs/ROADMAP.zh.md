...
清单，并把最终自检放入可信 system contract。DeepSeek-compatible 请求会显式保留
thinking/high effort，使用 4K 输出预算和可取消的 180 秒端到端硬时限；失败不会自动重试，
reasoning 原文不落库。本地只接受经过审查的 wrapper、嵌套及中英文常见字段别名，
...
1.14 已用独立外发契约消费上述 retrieval：人工必须再次确认所选 promoted memory 与绑定的
当前 canonical evidence 可以发送到显式配置的 provider-neutral 边缘。claim、debate、report
每个阶段都先经本地 permission-checked tools 重读全部当前证据；DeepSeek-compatible 配置保留
thinking/high effort，但 provider tools、原始 reasoning/响应持久化与自动重试保持关闭。结果仍是
非权威研究，不能自动召回、创建 memory、进入 Decision 或获得任何财务/券商/资本/执行权限。

1.15 再把“结构通过”和“人工认可”分开。人工必须对精确 1.14 输出记录接受为已复核研究、
要求修订或拒绝，并填写四项质量 rubric、事实错误/无证据主张数量，以及人工确认的定价快照
或明确缺价原因。系统复用 1.11 唯一的 citation/token/latency/成本口径，同时额外绑定 1.13
retrieval、promotion 与 selection 指纹、当前 context、report/artifact、provider/model/prompt 和
审计链。并发、重启和重复请求幂等；撤销或任一来源/证据/usage/审计漂移会撤销当前资格但保留
历史。接受仍不创建 memory、不自动召回、不进入 Decision，也没有财务、券商、资本或执行效果。
未来若要形成新 memory，必须另建显式、可撤销、版本化的 promotion 边界。

1.16 已实现该独立提升。只有当前回放仍有效的 1.15 `reviewed_research` 才能通过精确人工确认、
身份、理由和幂等键调用
`POST /api/ai/external-promoted-memory-analysis-reviews/{review_id}/memory-promotions`。
新历史 memory 绑定精确 review/analysis/report、1.13 retrieval 与源 promotion selections、当前
context、证据引用、provider/model/prompt、人工质量/成本指纹和审计链，并明确要求未来使用前
重新绑定当前证据。它使用独立表，不修改 1.12 schema，也不向源 workflow 增写 artifact。
