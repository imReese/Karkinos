...
| v1.8 adapter acceptance | Provider-neutral 基础已实现 | 版本化 manifest、capability/boundary matrix、append-only accept/reject/revoke review 与 live collector 精确绑定；尚未选择或注册真实 provider |
...
当前状态：provider-neutral release manifest 与 append-only review gate 已实现，包含 prepare/
commit 时的 drift 与 revocation 复核。真实 provider 选择、实际 ADR/威胁模型接受和 deployment
授权仍未完成，必须由项目所有者显式确认。
...
1. Adapter ADR、capability matrix、威胁模型和 acceptance manifest。
2. Provider-neutral conformance fixtures。
3. Provider 只读 adapter、collector integration、health 和 soak。
