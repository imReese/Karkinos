...
版本化 prompt 不关闭模型自身的推理模式，明确把嵌入字符串视为不可信数据，并给出允许引用
的 evidence id 与精确 JSON contract；本地只进行有界别名归一化，未知/缺失引用、截断、
畸形 JSON 或证据不完整都会 fail closed。系统只保存归一化 artifact、指纹、token、完成原因，
以及 reasoning 是否存在/长度，不保存原始响应、provider envelope、reasoning 原文或密钥。
...
后续按独立审查逐步迁移：下一步仅评估 1.10 输出的人工复核/处置边界，以及 provider 的质量、
延迟和成本证据；不得自动把外部结果晋升为 memory、Decision 或另一个 workflow。真实 provider
