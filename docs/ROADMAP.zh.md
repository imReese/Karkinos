...
再执行 exactly-once apply。Correction UI、更广的 fault-injection acceptance 与真实 provider
证据仍未完成，因此 M3 尚不是发布完成声明。
...
签名、原子 claim、只查询且防重复点击/restart 的 operator action。签名式 submission、correction
UI、真实 cancel、真实 adapter recovery 证据，以及完整的无需手改数据库旅程仍未完成。
