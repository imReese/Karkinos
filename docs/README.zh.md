...
Overview 的行情/净值复核只消费绑定 valuation snapshot、quote-set fingerprint、ledger cutoff/fingerprint 的 canonical 当前非零持仓投影；观察列表、大盘指数和已清仓事实不能抬高数量。同一个估值 identity 同时给出组合总状态以及股票、基金分层状态：基金确认净值延迟会让基金层和组合总估值降级，但不会把行情完整的股票层误标为降级，也不会阻断不使用账户总权益的纯研究；凡使用 `total_equity`、资本或组合风险的账户绑定流程仍要求组合总估值完整。
Market 会列出精确标的、原因和安全人工下一步，并把普通行情刷新与基金确认净值同步拆成不同的标的范围；Overview 使用相同的显式操作，不能用普通行情刷新冒充基金净值确认。GET 不联系 provider、不写库。基金确认净值使用独立、请求幂等的 confirmation-only 可审计 ingestion；同一请求只重放持久化批次且不再次联系 provider，估值和前一日净值不能清除复核。只有更新且已确认的持久化证据生成新快照后才能清除复核项。Overview 的“今日待办”把归一化 research preview 与账户 recommendation 分成两个只读证据区：前者始终标明不可执行、账户资格未评估，后者只展示 qualification 状态/阻断项以及基于当前账户事实生成的 buy、sell、hold、rebalance、review-required 或 first-class no-action。研究候选不能冒充账户建议，也不能清除账户阻断。纯股票 Formula discovery 不使用 `total_equity`，因此仍可独立于基金确认净值延迟；qualification、资本、组合风险和 Decision 使用总权益，组合 aggregate valuation 不完整时必须 fail closed。account-qualified candidate 经精确人工 paper/shadow-only 批准后，才会由下一交易日的 promoted scan 与独立 Decision 生成账户推荐；这不证明真实 provider、券商、paper/shadow 样本或 live financial readiness 已完成。
