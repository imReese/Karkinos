...
关闭。任务/复核幂等且可按哈希链回放，接受上下文也不会启动模型或 workflow。系统不
调用真实外部模型、不读取 API Key，也不默认绑定 DeepSeek 或其他厂商。
...
- **AI 原生投研运行基础** — 提供 provider/model/agent-role 分离注册、证据绑定的
  ...
  research task/review 会再次校验精确 context 并阻断不完整证据，当前仍不启动 AI
  workflow、不注册真实 provider、不提供交易权限
