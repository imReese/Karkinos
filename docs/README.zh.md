...
Karkinos 的方向是更专业的个人量化交易平台，而不是默认无人值守的实盘下单机器人。
v1.5 已把策略证据、组合状态、账户事实、风控、paper/shadow、费用与成本口径汇总成
每日交易计划和今日待办。系统可以准备候选池、阻断原因、目标仓位、预计数量、费用税费、
现金影响、约束检查和下一步复核动作，但默认仍需要人工确认，不会直接提交券商订单。

当前路线图的活跃阶段是 v1.6：强化 paper/shadow 运行手册、调度、差异报告和运行状态；v1.7 才考虑受控券商
执行桥接或订单票据导出。完全无人值守的真实资金自动下单仍属于延后范围。
