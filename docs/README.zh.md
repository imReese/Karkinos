...
当前路线图的活跃阶段是 v1.6：强化 paper/shadow 运行手册、调度、差异报告和运行状态；
v1.7 才考虑受控券商执行桥接或订单票据导出；v1.8 才考虑小资金、强上限、可暂停的
自动试点。完全无人值守的全账户真实资金自动下单仍属于延后范围。完整架构见
...
受控自动化、OMS、券商桥接、paper/shadow、对账和小资金自动试点的完整分层设计见
[Karkinos 架构](ARCHITECTURE.md)。
