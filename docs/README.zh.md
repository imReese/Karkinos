...
report 和待人工复核 memory；证据漂移会使绑定、回放和 memory 明确失效。系统不调用
人工还可对精确完成的分析选择接受为已复核研究记忆、要求修订或驳回；接受仅赋予 AI
研究域内的回忆资格，并在每次读取时重验 evidence、artifact 和审计链，漂移会自动撤销
资格。系统没有自动记忆检索或 Decision 交接，不调用真实外部模型、不读取 API Key、
不联网，也不默认绑定 DeepSeek 或其他厂商。
