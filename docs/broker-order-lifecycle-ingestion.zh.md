...
[券商适配器发布审查](broker-adapter-release-review.zh.md)。prepared run 在 commit 前会再次检查，
因此 restart recovery 期间发生的 review revocation 会阻断 lifecycle 写入与 cursor 前移。
...
- 同 cursor 不同 evidence、cursor 回退、跳号/乱序、deployment/release/authorization/account
  漂移会阻断；
- live batch 即使自报 `reviewed`，canonical adapter release review 缺失、rejected、revoked 或
  drifted 时仍会阻断；
...
通过版本化[发布审查边界](broker-adapter-release-review.zh.md)绑定依赖/来源审查、capability、威胁
