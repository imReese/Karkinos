...
3. 最终 Decision 与计划只能在上海时间 09:35 至 09:44 生成。每个订单意图使用当前持久化行情而不是历史信号价格，并绑定正数价格、来源、带时区时间和决策时行情年龄；该时间与 Decision、交易计划属于同一市场日期，且行情年龄不得超过 300 秒。
   在写入推荐任务前，已人工晋级策略还必须按前一已验证收盘日重放完整、逐日收据绑定的股票池；当前股票持仓走独立退出通道，基金与 ETF 不进入策略候选。系统只刷新已选中的新买入股票，重建 Account Truth 后再次扫描；两次信号选择 fingerprint 必须完全一致。
...
1. 在每日决策截止前由 collector 与每日快照滚动准备当前 Account Truth，并显式导入或复核当前行情。任何 GET 页面都不会隐式刷新 provider。物质性连续的 canonical 历史会自动继承账户范围：无活动快照滚动、历史非决策元数据整理、以及按时间追加的流水都不要求重复确认；新增股票成交只有在原费用规则下逐项匹配时自动复验，且不会扩大原复核金额包络。已有经济流水被改删、倒序补写、费用不匹配或资产范围改变才进入异常复核；每个中间 import 都会检查，恢复旧文件也不能绕过。
...
4. Karkinos 先以零信号写入方式扫描完整冻结股票池，刷新并审计入选买入标的的当前行情，重建 Account Truth，再执行一次完整扫描；只有信号集合完全不变时才持久化推荐任务。随后重建当前 Decision 与计划，运行 canonical 批量风控，持久化一次确定性 paper/shadow，再次重建计划后才给出生产结论。完整扫描没有任何买卖信号属于确定性的正常 `no_action`，不是“缺少策略”缺陷。
