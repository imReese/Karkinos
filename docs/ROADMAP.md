...
sanitized copy-only package plus an append-only, exactly-once acknowledgement that closes as no-retry. The v3 view prioritizes older critical unfinished journeys across every bounded persisted intent.
Signed submission, real signed cancel, real-adapter recovery evidence, and the complete journey remain open.
