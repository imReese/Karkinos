...
Owner-enabled no-activity snapshot roll-forward separates exact daily snapshot identity from `karkinos.account_truth.source_fact_lineage.v1`, a privacy-minimized fingerprint over every non-derived normalized statement row. `karkinos.account_truth.evidence_scope_review_binding.v1` and reviewed-fee preview v2 may inherit across derived trading-day snapshots only when the current lineage, reviewed account/window/assets, and every intervening import are identical. A new, corrected, removed, malformed, or temporarily divergent source fact breaks the lineage epoch; returning to an older fingerprint does not resurrect the prior review. The exact current import, snapshot capture, reconciliation, Account Truth promotion source, ledger cutoff, and daily ticket identity remain current-day gates. Inheritance is query-only, append-only-review-aware, explicitly revocable, and grants no broker, OMS, ledger, execution, or capital authority.
