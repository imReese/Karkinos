...
live scheduler 始终属于服务生命周期，不提供启停控制。自动化交易使用另一个默认关闭的运行时
总闸。操作员可以在不重启服务的情况下开启最长 12 小时，或立即关闭；每次变更都会绑定当前
revision、操作员、原因、确认语、过期时间和 fingerprint，并追加审计事件。

该门禁只约束 `session_bounded` 会话签发与订单准入。每个新会话或 replacement 都会在同一事务内
绑定当时 enabled 门禁的精确 revision 与 fingerprint。关闭会立即使后续准入失效；重新开启会形成
新的门禁代际，旧会话不能因此恢复准入。关闭不会取消或证明外部未决订单；`manual_each_order`、
查询、撤单、对账和账本恢复保持独立。全局 Kill Switch 仍拥有更高优先级。打开总闸本身不授予
资本或执行权限，也不能替代重新签名的 bounded session 或任何当前风控、预算、gateway 与对账
门禁。当前尚未实现 session 自动向券商提交订单。
