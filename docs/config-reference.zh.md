...
preview 可传已记录的 `capital_evaluation_input_fingerprint` 与明确的
`prior_batch_reconciliation_fingerprint`。确认请求还必须携带当前 `dossier_fingerprint`、
同一 prior-batch 指纹、与签名身份一致的 `operator_label`、对应 `operator_approval_id`，
以及固定 acknowledgement `confirm_exact_non_submitting_dossier_for_review`。接口拒绝额外
credential 字段。每次 preview 都从当前应用状态解析精确 `evidence_connector_id` 的签名
...
漂移会 fail closed。不同的 `execution_gateway_id` 会作为 runtime-unverified 证据单独绑定。
有效 promotion 只清除三个 Stage 1 子阻断，runtime authority、live gateway 和 broker
submission 仍保持 disabled，因此签名通过的 attestation 也不能成为券商命令。
...
`prior_batch_reconciliation_fingerprint`、1 至 50 个显式 `order_ids`，以及带时区的
`requested_start_at`/`requested_expires_at`。窗口最长 1800 秒，并且必须位于资本 policy
的有效期内。attestation 还需当前 `envelope_fingerprint`、与签名身份一致的
...
submit capability 会阻断，执行侧在独立 runtime verifier 落地前始终保持 unverified。
