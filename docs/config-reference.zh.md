...
# 可选：复制环境变量模板供 Docker Compose 使用
cp .env.example .env
python -m server
```

直接执行 `python -m server` 时，Python 不会自动读取 `.env`。请先把变量导出到当前进程环境，或通过 Docker Compose 启动；Docker Compose 会自动使用项目目录中的 `.env` 进行变量替换。
...
未知顶层字段、未知分组字段、同一字段同时使用新旧格式都会阻止启动。这样可以避免拼写错误被静默忽略。
...
`allow_financial_context` 不再作为推荐配置开关。是否允许发送某一类证据由具体的人工确认、不可变证据范围和对应 API 契约决定，不能由一个全局布尔值放宽。
...
布尔值接受 `true/false`、`1/0`、`yes/no` 和 `on/off`；拼写错误会阻止启动。Port、轮询间隔和 CORS 空列表同样会被校验。
