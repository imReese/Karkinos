...
Stage 3.6 要求 controlled-session envelope preview/attestation 额外提交
`per_symbol_runtime_limits` 对象。key 必须与本次 projected symbol 集合完全一致；value 为
正数 CNY 上限，最多四位小数，且不能高于已记录资本评估的 symbol/effective ceiling。该 map
...
Stage 4.4 的 v2 window 还会计算必需的 `execution_scope`：每笔样本订单必须绑定持久化
controlled-session admission 或 current clear exact batch。旧 v1 window 只能列出作历史审计，
当前评审必须重新记录 v2；该过程不会迁移权限、修改订单/账本或接触券商。
