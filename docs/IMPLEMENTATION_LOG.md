...
As of 2026-08-23, v0.3.0 is the stable local release baseline. The v1.8 control-plane foundation and AI-native research foundation through Phase 1.18 are implemented. The active product milestone is the broker-connected controlled per-order pilot in [ROADMAP.md](ROADMAP.md). The latest completed cross-cutting work includes:

- v0.3.0 replaces holdings-only AI research with `karkinos.market_universe_truth.v1`: one immutable, content-addressed full active A-share stock snapshot per provider/trading date; deterministic 160-name persisted-history backfill; and an exact reproducible 40-stock panel that excludes funds/ETFs and rejects missing, stale, short, invalid, untradable, or one-lot-infeasible data before any model call. DeepSeek now controls signal logic only: provider sizing is overwritten by the local fixed four-slot policy, signal-to-fill and lot-feasibility evidence enter canonical critique, and multi-asset equity uses each position's own marked value. The hourly after-close ingestion loop is idempotent, calendar-gated, rate-limited, resumable from persisted ready bars, provider-free on completed replay, and has no Account Truth, strategy-promotion, order, broker, execution, ledger, or capital-authority effect. Validation passed 2,519 Python tests, 697 Web tests, Web formatting and production build, documentation health, the strategy/broker boundary guard, target-file formatting/import checks, and a live 2026-08-21 TuShare ingestion/replay with 5,210 stock members, 159/160 persisted-ready histories, 78 capital/lot-eligible names, and one deterministic 40-stock panel;
