...
As of 2026-08-22, v0.2.5 is the stable local release baseline. The v1.8 control-plane foundation and AI-native research foundation through Phase 1.18 are implemented. The active product milestone is the broker-connected controlled per-order pilot in [ROADMAP.md](ROADMAP.md).

The latest completed cross-cutting work includes:

- v0.2.5 binds every DeepSeek hypothesis citation to a deterministic, fingerprinted catalog derived from the exact frozen sanitized export. Model-facing IDs are resolved locally to canonical evidence paths and revalidated against the frozen input; empty, oversized, unrepresentable, colliding, unknown, or drifted citations fail closed. Exact safe provider failure codes now survive the workflow and automation boundary without exposing raw responses. Operations Today separately distinguishes a stale-but-reconciled snapshot from a real Account Truth mismatch, suppresses staleness-only attention on non-trading days, and scopes paper/shadow counts to the current run. Validation passed 2,500 Python tests, 697 Web tests, Web formatting, the production build, documentation health, staged GitNexus impact checks, and pre-commit checks; it contacted no broker, submitted no order, changed no OMS/ledger/risk/capital fact, and did not consume the owner-authorized provider retry;
