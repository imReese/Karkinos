...
As of 2026-08-31, v0.3.2 is the current broker-provider-free software baseline. It retains the v0.3.1 database-upgrade and release-identity repairs, makes the live scheduler an unconditional service-lifecycle invariant, and keeps automatic trading behind a separate default-closed runtime gate that can change without restart. It also adds verified immutable macOS artifacts, external mutable state, journaled `current`/`previous` activation, readiness-bound restart, automatic rollback, and two-release local retention. The v1.8 control-plane foundation and AI-native research foundation through Phase 1.18 remain implemented, and the active product milestone is still the broker-connected controlled per-order pilot in [ROADMAP.md](ROADMAP.md). Scheduler liveness and software-release evidence grant neither capital authority nor automatic broker submission. The latest completed cross-cutting work includes:
