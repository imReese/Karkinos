...
As of 2026-08-22, v0.2.6 is the stable local release baseline. The v1.8 control-plane foundation and AI-native research foundation through Phase 1.18 are implemented. The active product milestone is the broker-connected controlled per-order pilot in [ROADMAP.md](ROADMAP.md).

The latest completed cross-cutting work includes:

- v0.2.6 includes the v0.2.5 deterministic, fingerprinted DeepSeek citation catalog and exact safe provider failure propagation, plus a market-date-bound Account Truth freshness clock for historical after-close research. The model-facing IDs still resolve locally to exact frozen evidence paths; empty, oversized, unrepresentable, colliding, unknown, or drifted citations fail closed. Weekend retries now evaluate the prior trading day's snapshot at that frozen date's authorized close and record the as-of timestamp, while ordinary current-state views remain wall-clock strict and missing, future-dated, stale-at-decision, drifted, or unreconciled evidence still blocks. Operations Today separately distinguishes stale-but-reconciled snapshots from real mismatches, suppresses staleness-only attention on non-trading days, and scopes paper/shadow counts to the current run. Validation passed 2,501 Python tests, 697 Web tests, Web formatting, the production build, documentation health, staged GitNexus impact checks, and pre-commit checks; it contacted no broker, submitted no order, changed no OMS/ledger/risk/capital fact, and did not consume the owner-authorized provider retry;
