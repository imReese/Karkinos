...
As of 2026-08-28, v0.3.1 is the current broker-provider-free software baseline. It repairs the exact known legacy v0.3.0 database upgrade state without rewriting migration provenance and makes SemVer plus `sha-*` container tags immutable while allowing release aliases such as `latest` to advance. The v1.8 control-plane foundation and AI-native research foundation through Phase 1.18 remain implemented, and the active product milestone is still the broker-connected controlled per-order pilot in [ROADMAP.md](ROADMAP.md). The live scheduler is now an unconditional service-lifecycle invariant, while a separate default-closed automatic-trading gate can be changed without restart for at most 12 hours. Its CAS/audit record and exact session revision, fingerprint, and disable-lineage binding prevent a close/reopen cycle from reviving old authority; it grants neither capital authority nor automatic broker submission. This supersedes older entries that described the scheduler or monitor as owner-enabled startup configuration. The latest completed cross-cutting work includes:

- the current untagged candidate fixes a local second-round citation-catalog overflow without weakening citation binding: hypothesis prompts now derive and validate only the four-or-five exported anchors instead of enumerating every unrelated nested evidence path. The workflow preserves the exact safe local failure code. For the already persisted corrected-panel run, an append-only provider-free partial-resume record binds the failed provider-call claim to the failed session, workflow, agent response, completed candidate lineage, prior/current runtime inputs, and exact checkpoint; it proves the failure occurred after local tool reads but before provider transport, excludes only that call from the external ceiling, and resumes at round two without regenerating round one. Admission additionally requires unchanged baseline/account identities and exactly enough previously authorized capacity for the remaining rounds. Validation passed 2,543 Python tests plus a current-database-copy migration/claim that preserved the run, restored one completed candidate, classified 17 real calls out of 25 recorded attempts before continuing, and retained the existing ceiling of 25. The path cannot promote a strategy, create or submit an order, mutate account/ledger evidence, or expand capital authority The same candidate also gives critique requests the same deterministic short citation-ID contract as hypotheses and adds one evidence-bound local resume for the exact corrected-panel first-critique failure. The owner extension is append-only and one-shot, requires the persisted 16-call/ceiling-24 lineage plus unchanged corrected-panel, baseline, valuation, ledger, session, draft, backtest, failed-critique, provider-call, and candidate evidence, and raises the ceiling only to 25. The resumed run reuses iteration one's completed hypothesis and local backtest, retries only its critique with a new authorization-bound idempotency key, then completes rounds two through five without regenerating earlier work. Any evidence drift fails closed. Validation passed 2,540 Python tests, targeted citation/route/resume tests, formatting and diff checks, and migration/claim against a current database copy with `PRAGMA quick_check=ok`; the prior runtime also opened that migrated copy safely. The path cannot promote a strategy, create or submit an order, mutate account/ledger evidence, or expand capital authority;
...
- a deterministic process-liveness-only endpoint plus scheduler-aware startup preflight distinguish a fully ready Karkinos service from an unresponsive, foreign, or scheduler-failed listener before build/launch, report without terminating it, and make no financial-readiness, provider, database-write, broker-action, ledger, execution, or capital-authority claim;
- an explicit macOS user-level LaunchAgent operations entry point renders its local definition before installation, runs the production backend with direct arguments on `127.0.0.1`, remains supervised after any process exit while loaded, verifies both process liveness and the always-on live scheduler, refuses to replace an existing listener, and supports exact reversible uninstall. It does not edit runtime configuration or establish financial, execution, broker, or capital readiness; the scheduler may contact the configured market-data provider;
