...
- **Web UI** — Vue 3 + ECharts dashboard with 6 real-time views
...
├── web/                    # Vue 3 frontend
│   ├── src/
│   │   ├── views/          # 6 views (dashboard/portfolio/market/signals/backtest/settings)
│   │   ├── components/     # Components (kline/equity-curve/allocation-pie etc.)
...
# Start server (default 0.0.0.0:8000, auto-starts live monitoring)
uv run python -m server

# Development mode (hot reload)
uv run python -m server --reload

# Disable auto-start of live monitoring
...
- **Stage 2** (`python:3.12-slim`): Installs uv, runs `uv pip install --system -e ".[server]"`, copies source + frontend dist, exposes port 8000, default command `python3 -m server`
...
services:
  myquant:
    ...
    ports:
      - "8000:8000"
    volumes:
      - myquant-data:/app/data/store    # Parquet + SQLite data persistence
