...
_IMAGE_REPOSITORY = re.compile(r"^ghcr\.io/[a-z0-9_.-]+/[a-z0-9_.-]+$")
...
def promote(
    *,
    ...
    if _IMAGE_REPOSITORY.fullmatch(image_reference) is None:
    ...
    if any(target.rsplit(":", 1)[0] != image_reference for target in targets):
        raise ValueError("stable_image_target_repository_mismatch")
    source = f"{image_reference}@{expected_digest}"
    if _inspect(source) != expected_digest:
