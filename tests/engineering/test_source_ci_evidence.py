...
class _FakeClient:
    def __init__(self, *, workflow=None, runs=None, jobs=None):
        ...
        self.jobs_payload = jobs or _jobs(_job("Full CI gate", 201))
...
def test_api_request_failure_is_inconclusive_and_closed(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tools.verify_release_source_ci.urlopen", fail_request)
...
def test_exact_successful_full_run_and_gate_are_accepted() -> None:
    client = _FakeClient()
    result = _wait(client)
    ...
    assert result.required_job_ids == (201,)
...
def test_latest_successful_job_result_is_accepted() -> None:
    jobs = _jobs(_job("Full CI gate", 301, run_attempt=2))
    result = _wait(_FakeClient(jobs=jobs))
    assert result.run_attempt == 2
    assert result.required_job_ids == (301,)
...
@pytest.mark.parametrize(
    ("jobs", "error"),
    (
        (_jobs(), "required_job_missing:Full CI gate"),
        (
            _jobs(_job("Full CI gate", 201), _job("Full CI gate", 203)),
            "required_job_ambiguous:Full CI gate",
        ),
        (
            _jobs(_job("Full CI gate", 201, conclusion="failure")),
            "required_job_not_success:Full CI gate",
        ),
        (
            _jobs(_job("Full CI gate", 201, head_sha="b" * 40)),
            "required_job_sha_mismatch:Full CI gate",
        ),
    ),
)
def test_required_job_evidence_failure_is_closed(jobs, error) -> None:
    with pytest.raises(SourceCIVerificationError, match=error):
        verify_required_jobs(
            deepcopy(jobs), required_job_names=_REQUIRED_JOBS, commit_sha=_SHA
        )
...
def test_incomplete_api_pagination_fails_closed(payload) -> None:
    if "workflow_runs" in payload:
        ...
        with pytest.raises(SourceCIVerificationError, match="pagination_incomplete"):
            verify_required_jobs(
                payload, required_job_names=_REQUIRED_JOBS, commit_sha=_SHA
            )
