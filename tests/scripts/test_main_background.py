...
def source(tmp_path):
    ...
    for name in ("config/config.json", "config/.env", "data/app.db", "data/meta.db"):
        (home / name).write_bytes(b"original fixture")
    ...
    driver.write_text(textwrap.dedent(f"""
        ...
        runtime.require_runtime_idle = lambda env: None
    ...
    env["KARKINOS_HOME"] = str(home)
    yield root, home, driver, env
    main_control.request_stop(root / ".run/main", timeout=5)
...
def test_default_start_returns_detached_with_logs_and_stop_allows_restart(source):
    ...
    assert str(home / "logs/main.log") in started.stdout
    pids = {
    ...
    assert not main_control._lock_held(root / ".run/main")
    assert not (root / ".run/main/control.sock").exists()
...
def test_preparation_failure_is_reported_and_logged_without_service(source):
    ...
    assert "fixture build failed" in (home / "logs/main.log").read_text()
...
def test_pending_start_cancellation_cleans_preparation_and_locks(source, interrupt):
    ...
    try:
        ...
        wait_for(lambda: not main_control._lock_held(root / ".run/main"))
        assert not (root / "api.pid").exists()
    finally:
