...
from server.routes.ai_external_reviewed_memory import create_router
...
class FixtureService:
    ...
    def _result(self, name, *args):
        ...
        return FixtureValue(
            {
                "promotion_id": "ai-external-memory-promotion-fixture",
...
def _client(monkeypatch, service):
    ...
    monkeypatch.setattr(
        "server.routes.ai_external_reviewed_memory."
...
def _promotion_payload():
    return {
        "idempotency_key": "external-memory-promotion-route-001",
        "promoted_by": "human:reese",
        "rationale": "保留为显式、可撤销的历史研究先例。",
        "confirmation": (
...
def _revocation_payload():
    return {
        "idempotency_key": "external-memory-revocation-route-001",
        "revoked_by": "human:reese",
        "reason": "撤销召回资格但保留审计历史。",
        "confirmation": (
...
@pytest.mark.unit
def test_external_reviewed_memory_routes_require_exact_confirmations(monkeypatch):
    ...
    assert (
        client.post(
            "/api/ai/external-analysis-reviews/ai-review/memory-promotions",
            json=promotion,
        ).status_code
        == 422
    )
    assert (
        client.post(
            "/api/ai/external-reviewed-memory-promotions/ai-memory/revocations",
            json=revocation,
        ).status_code
        == 422
    )
...
@pytest.mark.unit
def test_external_reviewed_memory_routes_are_explicit_local_and_read_lazy(
    monkeypatch,
):
    service = FixtureService()
    ...
    assert promoted.json()["legacy_retrieval_contract_modified"] is False
    ...
    listed = client.get(
        "/api/ai/external-reviewed-memory-promotions",
        params={"review_id": "ai-review", "limit": 10},
    )
    fetched = client.get("/api/ai/external-reviewed-memory-promotions/ai-memory")
    replayed = client.get(
        "/api/ai/external-reviewed-memory-promotions/ai-memory/replay"
    )
    ...
    assert listed.json()["legacy_retrieval_contract_modified"] is False
    assert listed.json()["provider_invocation_count"] == 0
    assert initialize_calls == [True, True, False, False, False]


@pytest.mark.unit
def test_external_reviewed_memory_routes_map_domain_errors(monkeypatch):
    service = FixtureService(
        error=ExternalReviewedMemoryPromotionRejected("source review drifted")
    )
    client, _ = _client(monkeypatch, service)
...
@pytest.mark.unit
def test_main_app_registers_external_reviewed_memory_routes():
    ...
    assert (
        "/api/ai/external-analysis-reviews/{review_id}/memory-promotions",
        "POST",
    ) in routes
    assert (
    ...
    assert ("/api/ai/external-reviewed-memory-promotions", "GET") in routes
    assert (
        "/api/ai/external-reviewed-memory-promotions/{promotion_id}/replay",
        "GET",
    ) in routes
