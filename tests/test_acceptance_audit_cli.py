...
def test_acceptance_audit_cli_controlled_session_runtime_rate_limiter_filter() -> None:
    result = _run_cli("--audit", "controlled_session_runtime_rate_limiter")

    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)

    assert payload["selected_audit"] == "controlled_session_runtime_rate_limiter"
    assert [audit["key"] for audit in payload["audits"]] == [
...
def test_acceptance_audit_cli_controlled_session_runtime_authority_filter() -> None:
    result = _run_cli("--audit", "controlled_session_runtime_authority")

    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)

    assert payload["selected_audit"] == "controlled_session_runtime_authority"
    assert [audit["key"] for audit in payload["audits"]] == [
...
def test_acceptance_audit_cli_controlled_session_live_gate_filter() -> None:
    result = _run_cli("--audit", "controlled_session_live_gate_orchestration")

    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)

    assert payload["selected_audit"] == "controlled_session_live_gate_orchestration"
    assert [audit["key"] for audit in payload["audits"]] == [
