    ...
    >
      <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="app-product-mark">{labels.accountStrategyKicker}</div>
      ...
      <div className="mt-4 flex min-w-0 flex-col gap-3 border-y border-[var(--app-divider)] py-3 lg:flex-row lg:items-center lg:justify-between">
        <p className="app-muted min-w-0 text-sm leading-6">
          {labels.accountStrategySelectedHint(selectedStrategyName)}
        </p>
        <div className="flex shrink-0 flex-wrap gap-2">
          <button
            className="app-button-secondary rounded-[var(--app-radius-control)] px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-60"
            disabled={
      ...
      <div className="mt-4 border-t border-[var(--app-divider)] pt-3">
        <p className="app-muted text-sm">{labels.accountStrategyPnlPending}</p>
        {assignment?.limitations?.length ? (
          <div className="mt-2 divide-y divide-[var(--app-divider)] border-y border-[var(--app-divider)]">
            {assignment.limitations.map((limitation) => (
              <p
                className="py-2.5 text-sm text-[var(--app-text)]"
                key={limitation}
              >
