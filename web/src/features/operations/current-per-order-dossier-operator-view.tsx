...
import type { CurrentPerOrderDossierOperatorController } from './current-per-order-dossier-operator-controller';
import { adapterReadOnlyStatusLabel } from './current-per-order-dossier-operator-model';

interface DossierViewProps {
  controller: CurrentPerOrderDossierOperatorController;
}

export function CurrentPerOrderDossierOperatorView({
  controller,
}: DossierViewProps) {
  return (
    <section
...
function DossierHeader({ controller }: DossierViewProps) {
  const { locale, open } = controller;
  return (
    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <div className="app-product-mark">
          {locale === 'zh' ? '受控执行证据' : 'Controlled execution evidence'}
        </div>
...
function DossierCandidateSelection({ controller }: DossierViewProps) {
  const { locale, candidates, candidateRows, effectiveOrderId, preview } =
    controller;
  return (
    <>
          ...
          <label className="grid min-w-0 gap-2 text-xs font-semibold text-[var(--app-soft)]">
            ...
            >
              {candidateRows.map((candidate) => (
                <option key={candidate.order_id} value={candidate.order_id}>
      ...
      {preview.isError ? (
        <div className="app-error-text mt-3 break-words text-sm" role="alert">
...
function DossierPreview({ controller }: DossierViewProps) {
  ...
  return (
    <div className="mt-4 min-w-0 rounded-2xl border border-[color-mix(in_srgb,var(--app-border)_30%,transparent)] p-3 sm:p-4">
      ...
      <div className="mt-3 grid min-w-0 gap-2 text-xs sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <EvidenceMetric
          label={locale === 'zh' ? '网关验证' : 'Gateway verification'}
          value={shortenedIdentity(
            preview.data.evidence_resolution
        ...
        <EvidenceMetric
          ...
          value={preview.data.capital_evaluation.scope.strategy_id || '—'}
        />
        <EvidenceMetric
          label={locale === 'zh' ? '账户别名' : 'Account alias'}
          value={preview.data.capital_evaluation.scope.account_alias || '—'}
        />
        <EvidenceMetric
          label={locale === 'zh' ? '适配器发布证据' : 'Adapter release'}
          value={shortenedIdentity(
            preview.data.broker_adapter_release.release?.release_evidence_ref ||
              '—',
          )}
        />
        <EvidenceMetric
          label={
            locale === 'zh' ? '适配器只读状态' : 'Adapter read-only status'
          }
        ...
        <EvidenceMetric
          label="kill switch"
          value={formatPublicStatus(preview.data.kill_switch.status, locale)}
        />
        <EvidenceMetric
      ...
      <DossierSignature controller={controller} />
    </div>
  );
}

function DossierSignature({ controller }: DossierViewProps) {
  const {
    locale,
    preview,
    approvalStatus,
...
function DossierFinalConfirmation({ controller }: DossierViewProps) {
  const { locale, verification, acknowledged, confirmation } = controller;
  return (
    <>
      {verification.data ? (
        <div className="mt-4 rounded-2xl border border-[color-mix(in_srgb,var(--app-warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--app-warning)_7%,transparent)] p-3">
          <label className="flex min-w-0 items-start gap-2 text-xs font-semibold text-[var(--app-text)]">
            <input
              ...
              type="checkbox"
              onChange={(event) =>
                controller.setAcknowledged(event.target.checked)
              }
            />
            <span>
              {locale === 'zh'
