...
import type { ControlledLedgerCorrectionOperatorController } from './controlled-ledger-correction-operator-controller';
import {
  correctionReasons,
  reasonLabel,
...
type ViewProps = {
  controller: ControlledLedgerCorrectionOperatorController;
};

export function ControlledLedgerCorrectionOperatorView({
  controller,
}: ViewProps) {
    ...
    <div className="mt-3 min-w-0 border-t border-[color-mix(in_srgb,var(--app-border)_28%,transparent)] pt-3">
      {!open ? (
        ...
        >
          ...
          <CorrectionInputs controller={controller} />
          <CorrectionPreview controller={controller} />
          <CorrectionSignature controller={controller} />
          <CorrectionFinalConfirmation controller={controller} />
        </section>
      )}
    </div>
  );
...
function CorrectionHeader({ controller }: ViewProps) {
  const { journey, locale, postingId } = controller;
  return (
    <>
...
function CorrectionInputs({ controller }: ViewProps) {
  const {
    approvalStatus,
    changeReason,
    effectiveKeyId,
    ...
    <>
      <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2">
        ...
        <label className="block min-w-0 text-xs font-semibold text-[var(--app-text)]">
          ...
          >
            {!eligibleIdentities.length ? <option value="">—</option> : null}
...
function CorrectionPreview({ controller }: ViewProps) {
  ...
  return (
    <div className="mt-3 min-w-0 rounded-2xl border border-[color-mix(in_srgb,var(--app-border)_32%,transparent)] p-3">
      ...
      <div className="mt-3 grid min-w-0 gap-2 text-xs sm:grid-cols-2 xl:grid-cols-3">
        ...
        >
          fingerprint: {shortenedIdentity(preview.data.correction_fingerprint)}
        </div>
        ...
        <div className="mt-3 min-w-0 rounded-xl border border-[color-mix(in_srgb,var(--app-border)_28%,transparent)] px-3 py-2">
          ...
          <div className="app-type-micro mt-2 grid min-w-0 gap-1 font-mono tabular-nums sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div>
              ...
              {preview.data.correction_plan.position_before.available_qty} →{' '}
              {preview.data.correction_plan.position_after.available_qty}
            </div>
            <div>
            ...
            <div>
              ...
              {preview.data.correction_plan.position_before.realized_pnl} →{' '}
              {preview.data.correction_plan.position_after.realized_pnl}
            </div>
            <div>
              {locale === 'zh' ? '累计费用' : 'Accumulated fees'}:{' '}
              {preview.data.correction_plan.position_before.commission_paid} →{' '}
              {preview.data.correction_plan.position_after.commission_paid}
            </div>
            <div>{locale === 'zh' ? '入金变化' : 'Deposit delta'}: 0</div>
...
function CorrectionSignature({ controller }: ViewProps) {
  const {
    challenge,
    changeSignature,
    createChallenge,
...
function CorrectionFinalConfirmation({ controller }: ViewProps) {
  const {
    acknowledged,
    apply,
    applyCorrection,
