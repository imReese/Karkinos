...
function renderCard(
  locale: 'en' | 'zh' = 'en',
  onSelectAssetClass?: (value: string) => void,
) {
...
test('renders overview holdings summary in english', () => {
  renderCard('en');
  expect(screen.getByText('Asset pulse')).toBeTruthy();
  expect(screen.getByText('Stock')).toBeTruthy();
  expect(screen.getByText('Market value')).toBeTruthy();
  expect(screen.getByText('Today move')).toBeTruthy();
});

test('renders overview holdings summary in chinese', () => {
  renderCard('zh');
  expect(screen.getByText('资产脉冲')).toBeTruthy();
