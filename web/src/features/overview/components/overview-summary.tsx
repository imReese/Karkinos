...
export function OverviewSummary({ summary }: { summary: AccountOverview }) {
  ...
  const cumulativeValue = financialValue(
  ...
  return (
    ...
    >
      <dl className="grid min-w-0 grid-cols-2 gap-y-5 md:grid-cols-[1.3fr_repeat(3,minmax(0,1fr))] md:gap-y-0 md:divide-x md:divide-[var(--app-divider)]">
        {metrics.map((metric, index) => (
          <div
            key={metric.key}
            className={`min-w-0 ${index === 0 ? 'md:pr-7' : 'md:px-7'} ${index === metrics.length - 1 ? 'md:pr-0' : ''}`}
          >
            <dt className="text-xs font-medium text-[var(--app-text-secondary)]">
              {metric.label}
            </dt>
            <dd
              {...(metric.testId ? { 'data-testid': metric.testId } : {})}
              className={`mt-1.5 text-[clamp(1.45rem,2.2vw,2rem)] leading-none font-semibold tracking-tight tabular-nums ${metric.tone}`}
            >
              {metric.value}
            </dd>
