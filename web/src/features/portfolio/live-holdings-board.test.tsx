...
test("renders live holdings board in english", () => {
  renderBoard("en");
  const copy = getCopy("en");

  expect(screen.getByText(copy.portfolio.liveBoard.title)).toBeTruthy();
  expect(screen.getByText("Example Fund")).toBeTruthy();
  expect(screen.getAllByText(copy.portfolio.liveBoard.latestPrice).length).toBeGreaterThan(0);
  expect(screen.getAllByText(copy.portfolio.liveBoard.todayMove).length).toBeGreaterThan(0);
  expect(screen.getAllByText(copy.portfolio.liveBoard.sinceBuyReturn).length).toBeGreaterThan(0);
  expect(screen.getByText(copy.portfolio.liveBoard.quoteLive)).toBeTruthy();
});

test("renders localized live holdings board in chinese", () => {
  renderBoard("zh");
