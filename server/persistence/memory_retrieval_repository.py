"""Read repository and audit replay for reviewed-memory retrieval."""
...
class ReviewedMemoryRetrievalRepositoryMixin:
    _path: Path
    ...
    def _get(self, retrieval_id: str) -> StoredReviewedMemoryRetrieval:
        try:
            with self._connection() as conn:
                row = conn.execute(
                    "SELECT * FROM ai_reviewed_memory_retrievals "
    ...
    def _list(self, *, limit: int = 50) -> tuple[StoredReviewedMemoryRetrieval, ...]:
    ...
    def _verify_replay(
        ...
        for expected_sequence, row in enumerate(rows, start=1):
            ...
            expected_hash = self._retrieval_event_hash(
