...
| `uv run python scripts/audit_daily_candidate_production.py --pretty` | Read the running local service's exact financial preflight, monitor, five-round research policy, and 20-day / 50-order trial into one sanitized readiness report. | Loopback GET only; no provider/broker contact or database write. Exit `0` means ready to continue bounded forward paper/shadow collection, not GO, profit, execution, or capital authority; exit `2` is fail-closed non-ready. |
